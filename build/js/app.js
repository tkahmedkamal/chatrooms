...
addUsername.addEventListener("submit", function (event) {
  event.preventDefault(); // Get Input
...
var storagUsername = localStorage.username ? localStorage.username : "unknown";
/* ============================================
...
roomList.addEventListener("click", function (event) {
  if (event.target.id === "check") {
    ...
    var roomName = event.target.parentElement.id; // Header > Updata chat room name

    roomNewName.textContent = roomName; // Remove all messages when we choose chat room

    messagesList.querySelectorAll("li").forEach(function (i) {
      i.remove();
    }); // Update chat room name

    chat_rooms.updateRoom(roomName); // Display data for the new chat room

    chat_rooms.getData(function (passData) {
      return inter_face.displayData(passData);
    });
  }
});
/* ============================================
...
var chat_rooms = new Chatrooms(storagUsername, "public"),
